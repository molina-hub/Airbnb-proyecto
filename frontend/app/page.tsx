...
function TopPropiedades() {
  const [ciudadTop, setCiudadTop] = useState("");
  const [top, setTop] = useState<{ id: number; titulo: string; promedio: number; cantidad_resenas: number; precio_noche: number }[]>([]);
  const [mensaje, setMensaje] = useState("");
  async function consultarTop(event: FormEvent<HTMLFormElement>) { event.preventDefault(); if (!ciudadTop.trim()) { setMensaje("Ingresá una ciudad para ver el ranking."); return; } try { setMensaje(""); setTop(await api(`/propiedades/top?ciudad=${encodeURIComponent(ciudadTop.trim())}`)); } catch (cause) { setMensaje(errorMessage(cause)); } }
  return <section className="mx-auto max-w-7xl px-6 pb-14"><div className="rounded-3xl bg-slate-900 p-8 text-white"><h2 className="text-3xl font-bold">Top 10 alojamientos mejor valorados</h2><p className="mt-2 text-slate-200">Conocé las propiedades con al menos tres opiniones en tu ciudad.</p><form onSubmit={consultarTop} className="mt-5 flex max-w-xl gap-3"><input className="campo" value={ciudadTop} onChange={(event) => setCiudadTop(event.target.value)} placeholder="Ej: Córdoba" /><button className="rounded-xl bg-rose-600 px-5 font-bold text-white hover:bg-rose-700">Ver ranking</button></form>{mensaje && <p role="alert" className="mt-4 text-amber-200">{mensaje}</p>}{top.length > 0 && <ol className="mt-6 grid gap-3 md:grid-cols-2">{top.map((propiedad, index) => <li key={propiedad.id} className="rounded-xl bg-white p-4 text-slate-950"><strong>#{index + 1} · {propiedad.titulo}</strong><p>★ {Number(propiedad.promedio).toFixed(1)} · {propiedad.cantidad_resenas} opiniones · ${Number(propiedad.precio_noche).toLocaleString("es-AR")}/noche</p></li>)}</ol>}</div></section>;
}
