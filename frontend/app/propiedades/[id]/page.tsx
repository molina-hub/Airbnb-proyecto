...
type Propiedad = { id: number; titulo: string; ciudad: string; direccion: string; descripcion?: string | null; imagen_url?: string | null; precio_noche: number; capacidad: number; anfitrion_id: number; amenidades: { id: number; nombre: string }[] };
...
function CalendarioDisponibilidad({ propiedadId }: { propiedadId: number }) {
  const [mes, setMes] = useState(new Date().toISOString().slice(0, 7));
  const [dias, setDias] = useState<{ fecha: string; estado: "libre" | "ocupado" }[]>([]);
  const [error, setError] = useState("");
  const cargar = useCallback(async () => { try { setError(""); const data = await api<{ dias: { fecha: string; estado: "libre" | "ocupado" }[] }>(`/propiedades/${propiedadId}/disponibilidad?mes=${mes}`); setDias(data.dias); } catch (cause) { setError(errorMessage(cause)); } }, [propiedadId, mes]);
  useEffect(() => { const timer = window.setTimeout(() => { void cargar(); }, 0); return () => window.clearTimeout(timer); }, [cargar]);
  return <section className="rounded-2xl border border-slate-200 bg-white p-5"><div className="flex items-center justify-between gap-3"><h2 className="text-xl font-bold text-slate-950">Disponibilidad</h2><input aria-label="Mes de disponibilidad" type="month" className="rounded-lg border border-slate-300 p-2" value={mes} onChange={(event) => setMes(event.target.value)} /></div>{error ? <p role="alert" className="mt-3 text-sm text-amber-900">{error}</p> : <div className="mt-4 grid grid-cols-4 gap-2">{dias.map((dia) => <span key={dia.fecha} title={dia.estado} className={`rounded-lg p-2 text-center text-xs font-semibold ${dia.estado === "ocupado" ? "bg-rose-100 text-rose-900" : "bg-emerald-100 text-emerald-900"}`}>{dia.fecha.slice(-2)}<br />{dia.estado}</span>)}</div>}<p className="mt-3 text-sm text-slate-700">Verde: libre · Rosa: ocupado</p></section>;
}
