...
function ResenasPropiedad({ propiedadId, usuarioId }: { propiedadId: number; usuarioId?: number }) {
  ...
  const elegibles = reservas.filter((reserva) => reserva.propiedad_id === propiedadId && (["confirmada", "finalizada"].includes(reserva.estado) || reserva.fecha_fin <= hoy) && !resenas.some((resena) => resena.reserva_id === reserva.id));
