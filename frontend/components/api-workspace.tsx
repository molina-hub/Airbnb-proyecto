...
import { ApiError, api, errorMessage, hasSessionToken } from "../lib/api";
...
type Reserva = { id: number; propiedad_id: number; fecha_inicio: string; fecha_fin: string; estado: string; total: number; propiedad: { titulo: string; ciudad: string }; huesped: { id: number; nombre: string; email: string }; anfitrion: { id: number; nombre: string } };
...
export default function ApiWorkspace({ mode }: { mode: Mode }) {
  ...
  const cargar = useCallback(async () => {
    ...
    try {
      ...
      if (mode === "mispropiedades") {
        const [listaPropiedades, listaReservas] = await Promise.all([api<Propiedad[]>(`/anfitriones/${hostId}/propiedades`), api<Reserva[]>(`/anfitriones/${hostId}/reservas`)]);
        setPropiedades(listaPropiedades); setReservas(listaReservas);
      }
  ...
  const ejecutar = async (action: () => Promise<unknown>, ok: string) => {
    setError("");
    setProcesando(true);
    try { setResultado(await action()); setMensaje(ok); await cargar(); } catch (cause) { if (cause instanceof ApiError && cause.status === 401) { cerrarSesion(); setError("Tu sesión venció. Iniciá sesión para continuar."); window.setTimeout(() => window.location.assign("/registro"), 700); } else setError(errorMessage(cause)); }
    finally { setProcesando(false); }
  };

  const actualizarEstadoReserva = (reserva: Reserva, estado: "confirmada" | "rechazada" | "cancelada") => {
    const etiqueta = estado === "confirmada" ? "confirmada" : estado === "rechazada" ? "rechazada" : "cancelada";
    void ejecutar(
      () => api(`/reservas/${reserva.id}/estado`, { method: "PATCH", body: JSON.stringify({ estado, anfitrion_id: hostId }) }),
      `Reserva ${etiqueta} con éxito.`,
    );
  };
  ...
  const contenido = () => {
    ...
    if (mode === "top") return <><input value={ciudad} placeholder="Ciudad" onChange={(e) => setCiudad(e.target.value)} /><button className="ml-3 rounded bg-rose-500 px-4 py-2 text-white" onClick={() => { if (!ciudad.trim()) { setError("Ingresá una ciudad para consultar el ranking."); return; } void ejecutar(() => api(`/propiedades/top?ciudad=${encodeURIComponent(ciudad.trim())}`), "Ranking actualizado."); }}>Buscar</button><Listado datos={resultado} /></>;
    if (mode === "reservar") return <ReservaForm propiedades={propiedades} seleccion={seleccion} setSeleccion={setSeleccion} fechaInicio={fechaInicio} setFechaInicio={setFechaInicio} fechaFin={fechaFin} setFechaFin={setFechaFin} userId={userId} ejecutar={ejecutar} setError={setError} />;
    ...
    if (mode === "mispropiedades") return <div className="space-y-7"><div className="space-y-3">{propiedades.map((p) => <article key={p.id} className="rounded-xl border border-slate-200 p-4 text-slate-950"><strong>{p.titulo}</strong> — {p.ciudad} — ${p.precio_noche}<button className="ml-3 rounded bg-red-600 px-3 py-1 text-white" onClick={() => void ejecutar(() => api(`/propiedades/${p.id}`, {method: "DELETE"}), "Propiedad eliminada.")}>Eliminar</button></article>)}{propiedades.length === 0 && <p className="text-slate-800">No hay propiedades publicadas.</p>}</div><IngresosAnfitrion anfitrionId={hostId} /><div className="rounded-2xl border border-slate-200 bg-slate-50 p-5"><h2 className="mb-1 text-xl font-bold text-slate-950">Reservas recibidas</h2><p className="mb-4 text-slate-700">Administrá las solicitudes de reserva de tus alojamientos.</p><Reservas reservas={reservas} accion={actualizarEstadoReserva} /></div></div>;
    if (mode === "historial") return <Reservas reservas={reservas} />;
    if (mode === "gestion") return <Reservas reservas={reservas} accion={actualizarEstadoReserva} />;
...
function IngresosAnfitrion({ anfitrionId }: { anfitrionId: number }) { const [desde, setDesde] = useState(new Date(new Date().getFullYear(), new Date().getMonth(), 1).toISOString().slice(0, 10)); const [hasta, setHasta] = useState(new Date().toISOString().slice(0, 10)); const [datos, setDatos] = useState<{total_facturado: number; detalle: {propiedad_id:number; titulo:string; total_facturado:number; cantidad_reservas:number}[]} | null>(null); const [error, setError] = useState(""); const consultar = async () => { if (desde > hasta) { setError("La fecha inicial no puede ser posterior a la final."); return; } try { setError(""); setDatos(await api(`/anfitriones/${anfitrionId}/ingresos?desde=${desde}&hasta=${hasta}`)); } catch (cause) { setError(errorMessage(cause)); } }; return <section className="rounded-2xl border border-slate-200 bg-white p-5"><h2 className="text-xl font-bold text-slate-950">Ingresos del anfitrión</h2><div className="mt-4 flex flex-wrap gap-3"><label className="font-semibold text-slate-900">Desde<input className="ml-2 rounded border border-slate-300 p-2" type="date" value={desde} onChange={(event) => setDesde(event.target.value)} /></label><label className="font-semibold text-slate-900">Hasta<input className="ml-2 rounded border border-slate-300 p-2" type="date" value={hasta} onChange={(event) => setHasta(event.target.value)} /></label><button type="button" onClick={() => void consultar()} className="rounded bg-rose-600 px-4 py-2 font-bold text-white">Consultar</button></div>{error && <p role="alert" className="mt-3 text-amber-900">{error}</p>}{datos && <div className="mt-4"><p className="text-lg font-bold text-slate-950">Total facturado: ${Number(datos.total_facturado).toLocaleString("es-AR")}</p>{datos.detalle.map((item) => <p key={item.propiedad_id} className="text-slate-800">{item.titulo}: ${Number(item.total_facturado).toLocaleString("es-AR")} · {item.cantidad_reservas} reservas</p>)}{datos.detalle.length === 0 && <p className="text-slate-700">No hubo reservas confirmadas finalizadas en este período.</p>}</div>}</section>; }
function ReservaForm({ propiedades, seleccion, setSeleccion, fechaInicio, setFechaInicio, fechaFin, setFechaFin, userId, ejecutar, setError }: { propiedades: Propiedad[]; seleccion: number; setSeleccion: (value: number) => void; fechaInicio: string; setFechaInicio: (value: string) => void; fechaFin: string; setFechaFin: (value: string) => void; userId: number; ejecutar: (action: () => Promise<unknown>, ok: string) => Promise<void>; setError: (value: string) => void }) { const reservar = () => { if (!hasSessionToken()) { setError("Debés iniciar sesión antes de solicitar una reserva."); window.setTimeout(() => window.location.assign("/registro"), 700); return; } if (!seleccion || !fechaInicio || !fechaFin) { setError("Elegí un alojamiento y completá ambas fechas."); return; } if (fechaFin <= fechaInicio) { setError("La fecha de salida debe ser posterior a la fecha de entrada."); return; } void ejecutar(() => api("/reservas", {method: "POST", body: JSON.stringify({propiedad_id: seleccion, huesped_id: userId, fecha_inicio: fechaInicio, fecha_fin: fechaFin})}), "Reserva creada en estado pendiente."); }; return <div className="grid gap-4 rounded-2xl border border-slate-200 bg-slate-50 p-5 text-slate-950 sm:grid-cols-3"><label className="font-semibold text-slate-900">Alojamiento<select className="mt-1 w-full rounded-lg border border-slate-300 p-2 text-slate-950" value={seleccion} onChange={(e) => setSeleccion(Number(e.target.value))}>{propiedades.map((p) => <option key={p.id} value={p.id}>{p.titulo} — ${p.precio_noche}/noche</option>)}</select></label><label className="font-semibold text-slate-900">Entrada<input className="mt-1 w-full rounded-lg border border-slate-300 p-2 text-slate-950" type="date" value={fechaInicio} onChange={(e) => setFechaInicio(e.target.value)} /></label><label className="font-semibold text-slate-900">Salida<input className="mt-1 w-full rounded-lg border border-slate-300 p-2 text-slate-950" type="date" min={fechaInicio || undefined} value={fechaFin} onChange={(e) => setFechaFin(e.target.value)} /></label><button type="button" className="rounded-xl bg-rose-600 px-4 py-3 font-bold text-white hover:bg-rose-700 sm:col-span-3" onClick={reservar}>Solicitar reserva</button></div>; }
function Reservas({ reservas, accion }: { reservas: Reserva[]; accion?: (reserva: Reserva, estado: "confirmada" | "rechazada" | "cancelada") => void }) { return <div className="space-y-3">{reservas.map((r) => <article className="rounded-xl border border-slate-200 bg-white p-4 text-slate-950" key={r.id}><strong>{r.propiedad.titulo}</strong><p className="mt-1 text-slate-800">Huésped: {r.huesped?.nombre ?? "No disponible"}{r.huesped?.email ? ` (${r.huesped.email})` : ""}</p><p className="text-slate-800">{r.fecha_inicio} a {r.fecha_fin} · Total: ${r.total}</p><p className="mt-1 font-semibold capitalize text-slate-950">Estado: {r.estado}</p>{accion && r.estado === "pendiente" && <div className="mt-3"><button className="mr-2 rounded bg-green-700 px-3 py-1 text-white hover:bg-green-800" onClick={() => accion(r, "confirmada")}>Confirmar</button><button className="rounded bg-red-600 px-3 py-1 text-white hover:bg-red-700" onClick={() => accion(r, "rechazada")}>Rechazar</button></div>}{accion && r.estado === "confirmada" && <button className="mt-3 rounded bg-slate-800 px-3 py-1 text-white hover:bg-slate-950" onClick={() => accion(r, "cancelada")}>Cancelar reserva</button>}</article>)}{reservas.length === 0 && <p className="rounded-xl bg-white p-4 text-slate-800">No hay reservas recibidas.</p>}</div>; }
